...
if exist(densePE_matname, 'file') ~= 2
    ...
    for ii = 1:1:length(ImgList_original)
        ...
        if exist(q_densefeat_matname, 'file') ~= 2
            cnn = at_serialAllFeats_convfeat(net, query_dir, ImgList_original(ii).queryname, 'useGPU', false);
            cnn{1} = [];
            cnn{2} = [];
%             cnn{4} = [];
        ...
        for jj = 1:1:shortlist_topN
            ...
            if exist(db_densefeat_matname, 'file') ~= 2
                cnn = at_serialAllFeats_convfeat(net, db_dir, ImgList_original(ii).topNname{jj}, 'useGPU', false);
                cnn{1} = [];
                cnn{2} = [];
%                 cnn{4} = [];
    ...
    for ii = 1:1:length(ImgList_original)
        ...
        for kk = 1:1:shortlist_topN%parfor
...
disp('end:ht_top100_densePE_localization');
