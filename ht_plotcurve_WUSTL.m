...
for ii = 1:1:length(method)
    ...
    for jj = 1:1:length(DUC1_RefList)
        this_locid = strcmp([DUC1_RefList(jj).queryname '.png'], {ImgList.queryname});
        if sum(this_locid) == 0
            poserr_DUC1(jj) = inf;
            orierr_DUC1(jj) = inf;
            
            disp(DUC1_RefList(jj).queryname);
            disp(ImgList(1).queryname);
    ...
    for jj = 1:1:length(DUC2_RefList)
        this_locid = strcmp([DUC2_RefList(jj).queryname '.png'], {ImgList.queryname});
        if sum(this_locid) == 0
            poserr_DUC2(jj) = inf;
            orierr_DUC2(jj) = inf;
            keyboard;
